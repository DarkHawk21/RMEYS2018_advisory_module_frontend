...
export const useUserStore = defineStore('user', {
  ...
  actions: {
    async login(loginData) {
      ...
      try {
        const { data } = await axios.post('http://192.168.1.68:8000/api/v1/login', loginData);
    ...
    async logout() {
      ...
      try {
        const response = await axios.post('http://192.168.1.68:8000/api/v1/logout', this.token);
